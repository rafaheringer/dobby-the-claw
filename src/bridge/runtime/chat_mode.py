    ...
    try:
        while True:
            ...
            while True:
                ...
                if sleeping:
                    logging.info("[%dms] Ignoring text input while sleeping; say wakeword to wake", _elapsed_ms())
                    continue
                ...
                _on_user_text(text)
                sent = realtime.send_text(text) if realtime is not None else False
                if not sent:
            ...
            if idle_sleep_enabled and (not sleeping) and (now - last_user_activity) >= idle_sleep_timeout_s:
                _enter_sleep_mode()
